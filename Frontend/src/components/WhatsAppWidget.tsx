...
export default function WhatsAppWidget() {
  ...
  const message = encodeURIComponent(
    'Hello Dr. Kajal, I would like to book a consultation.'
  );
  const waLink = `https://wa.me/${phoneNumber}?text=${message}`;

  return (
    <div className="fixed bottom-24 lg:bottom-8 right-5 z-50 group">
      <a
        href={waLink}
        target="_blank"
        rel="noopener noreferrer"
        className="relative w-14 h-14 rounded-full bg-gradient-to-br from-[#25D366] to-[#128C7E] flex items-center justify-center shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300"
